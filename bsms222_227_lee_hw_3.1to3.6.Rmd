...
avg <- function(x, arithmetic = TRUE){
  n <- length(x)
  ifesle(arithmetic, sum(x)/n, prod(x)^(1/n))
}
```
...
The formula for the sum of the series ~ . What if we weren’t sure that was the right function? How could we check? Using what we learned about functions we can create one that computes the Sn:
```{r}
...
## 3.5 Vectorization and functinoals
...
3. The function ```nchar``` (자릿수를 의미함)tells you how many characters long a character vector is. Write a line of code that assigns to the object ```new_names``` the state abbreviation when the state name is longer than 8 characters.
```{r}
library(dslabs)
data("murders")
```
```{r}
ind <- which(nchar(murders$state) >8)
ind
...
12. Confirm that the forula for this sum is Sn = n(n+1)(2n+1)/6
```{R}
