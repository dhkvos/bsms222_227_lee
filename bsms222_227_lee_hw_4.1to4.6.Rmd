...
Although we have overwritten the original ```murders``` object, this does not change the object that loaded with ```data(murders)```. If we load the ```murders``` data again, the original will overwrite our mutated version. ('mutate'는 비파과적. murders라는 변수는 변할 수 있지만 data(murders)라는 원본은 변하지 않음)
...
```{r}
my_states <- filter(murders, region %in% c("Norhteast", "West") & rate < 1)
