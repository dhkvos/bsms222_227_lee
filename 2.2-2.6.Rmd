...
Suppose a high school student asks us for help solving several quadratic equations of the form ~. The quadratic formula gives us the solutions:~ which of course change depending on the values of a,b, and c. One advantage of programming languages is that we can define variables and write expressions with these variables, similar to how we do so in math but obtain a numeric solution.l We will write out general code for the quadratic equation below, but if we are asked to solve ~, then we define:
```{r}
a <- 1
b <- 1
c <- -1
...
a
```
A more explicit way to ask R to show us the value stored in ```a``` is useing ```print``` like this:
```{r}
print(a)
```
We use the term object to descrive stuff that is stored in R. Variables are examples, but objects can also be more complicated entities such as functions, which are described later. 

2.2.2 The workspace

As we define objects in the console, we are actually changing the workspace. You can see all the variable saved in your workspace by typing:
```{r}
ls()
```
...
Now since these values are saved in variables, to obtain a solution to our equation, we use the qudratic formula:
```{r}
...
We already used the ```install.packages```, ```library```, and ```ls``` functions. We also used the function ```sqrt``` to solve the qudratic equation above. There are many nore prebuilt functions and even more can be added through packages. These functions do not appear in the workspace because you did not define them, but they are available for immediate use. In general, we need to use parenthese to evaluate a function. If you type ```ls```, the function in not evaluated and instead R shows you the code that defines the function. If you type ```ls()``` the function is evaluated and, as seen above, we see objects in the workspace. 
Unlike ```ls```, most functions require one or more arguments. Below is and wxample of how we assign an object to the argument of the function ```log```. Remember that we earlier defined ```a``` to be 1:
```{r}
log(8)
log(a)
```
You can find out what the function expects and what it does by reviewing the very useful manuals inclued in R. You can get help by using the ```help``` function like this:
```{r}
help("log")
```
For most functionsm, we can also use this shorthand:
```{r}
?log
```
The help page will show you what arguments the function in expecting. For example, ```log``` needs ```x``` and ```base``` to run. However, some arguments are required and others are optional. You can determine which arguments are optional by noting in the help document that a default value is assigned with ```=```.  
...
There are several datasets that are included for users to practive and test out functions. You can see all the available datasets by typing:
```{r}
data()
```
...
Other prebuilt objects are mathematical quantities, such as the constant pi and infi:
```{r}
pi
Inf +1
```
...
A nice convention to foolow is to use meangingful words that describe what is stored, use only lower case, and use underscores as a substitute for spaces. For the quadratic equations, we could use something like this:
```{r}
...
We actually recommend against saving the workspace this way because, as you start working on different projects, it will become harder to keep track of what is saved. Instead, we recommend you assign the workspace a specific name. You can do this by using the function ```save``` or ```save.image```. 
To load, use the function ```load```. When saving a workspace, we recommend the suffix ```rda``` or ```RData```. 
In RStudio, you can also do this by navigating to the Session tab and choosing Save Workspace as. You can later oad it using the Load Workspace options in the same tab. You can read the help pages on ```save```, ```save.image```, and ```load``` to learn more. 
...
2.2.8 Commmenting your code

If a line of R code starts with the synbol ```#```, it is not evalueated. We can use this to write reminders of why we wrote particular code. For example, in the script above we could add:
```{r}
...
1. What is the sum of the first 100 positive integers? The frmula for the sum of integers 1 through n is n(n+1)/2. Define n = 100 and then use R to compute the sum of 1 through 100 using the formula. What is the sum?
```{r}
# my answer
n <- 100
sum_n = n*(n+1) / 2
...
# 500500
```
Based on the result, what do you think the functions ```seq``` and ```sum``` so? You can use ```help```.
...
A large proportion of data analysis challenges start with data stored in a data frame. For example, we stored the data for our motivation example in a data frame. You can access this dataset by loading the dslabs library and loading the ```murders``` dataset using the ```data``` function:
```{r}
...
Tip: R comes with a very nice auto-complete functionality that save us the trouble of typing out all the names. Try typing ```murders$p``` then hitting the tab key on your keyboard. This functionality and many other useful auto-complete features are available when working in RStudio. 
...
class(murders$state)
```
As with numeric vectors, all etries in a character vector need to be a character. 

Anorther important type of vectors are logical vectors. These must be either ```TRUE``` or ```FALSE```.
```{r}
z <- 3 ==2
z
class(z)
```
Here the ```==``` is a relational operator asking if 3 is equal to 2. In R, if you just use one ```=```k you actually assign a variable, but if you use two ```==``` you test for equality. 
...
Data frames are a special case of lists. We well cover lists in more detail later, but know that they are useful because you can store any combination of different types. Below is and example of a list we created for you:
```{r}
record <- list()
...
record2
```
If a list does not have names, you cannot extract thee elements with ```$```, but you can still use the brackets method and instead of providing the variable name, you provide the list index, like this:
```{r}
record2[[1]]
```
...
Matrices are another type of object that are common in R. Matrices are similar to data frames in that they are two-dimensional: they have rows and columns. However, like numeric, character and logical vectors, entries in matarices have to be all the same type. For this reason data frames are much nore useful for storing data, since we can have characters, factors, and numbers in them.

Yet matrices have a major advantage over data frames: we can perform matrix algebra operations, a powerful type of mathematical technique. We do not descrivbe these operations in this book, but much of what happens in the background when you perform a data analysis involves matrices. We cover matrices in more detail in Chapter 33.1 but describe them briefly here since some of the functions we will learn return matrices. 
...
mat
```
You can access specific entries in a matrix using squarwe brackets(```[```). If you wnat the second row, third column, you use:
```{r}
mat[2,3]
```
...
mat[1:2, 2:3]
```
We can convert matarices into data frames using the function ```as.data.frame```:
```{r}
as.data.frame(mat)
```
...
6. The function ```table``` takes a vector and returns the frequency of each element. You can quickly see how many states are in each region by applying this function. Use this function in one line of code to create a table of state per region. 
```{r}
table(murders)
```
...
```{r}
codes <- c(italy = 300, canada = 124, egypt = 818)
codes
```
...
class(1:10)
```
However, if we create a sequene including non-integers, the class changes:
```{r}
class(seq(1,10,0.5))
```
...
codes[2]
```
You can get more than one entry by using a muli-entry vector as an index:
```{r}
codes[c(1,3)]
```
The sequences defined above are particularly useful if we want to access, saym the first two elements:
```{r}
codes[1:2]
```
