...
```{r}
ref <- NHANES %>% filter( Gender %in% "female" & AgeDecade %in% " 20-29") %>% summarize(average = mean(BPSysAve, na.rm=T), standard_deviation = sd(BPSysAve, na.rm = T) )
ref
```
...
```{r}
NHANES$BPSysAve %>% min(na.rm=T)
NHANES$BPSysAve %>% max(na.rm=T)
```
...
```{r}
NHANES %>% filter(Gender %in% "female") %>% group_by(AgeDecade) %>% summarize(average = mean(BPSysAve, na.rm=T), standard_deviation = sd(BPSysAve, na.rm = T) )
```

5. Repeat exercise 4 for males.
```{r}
NHANES %>% filter(Gender %in% "male") %>% group_by(AgeDecade) %>% summarize(average = mean(BPSysAve, na.rm=T), standard_deviation = sd(BPSysAve, na.rm = T) )
```
...
```{r}
NHANES %>% group_by(AgeDecade,Gender) %>% summarize(average = mean(BPSysAve, na.rm=T), standard_deviation = sd(BPSysAve, na.rm = T) )
```
...
```{r}
NHANES %>% filter(Gender %in% "male" & AgeDecade %in% " 40-49") %>% group_by(Race1) %>% arrange(BPSysAve) %>% select(ID, Gender, Age, AgeDecade, Race1, BPSysAve)
```
...
class(murders[,4])
```
is not a data frame. With tibbbles this does not happen:
```{r}
...
```{r}
murders %>% select(population) %>% log() %>% .$population %>% mean() %>% exp
```
