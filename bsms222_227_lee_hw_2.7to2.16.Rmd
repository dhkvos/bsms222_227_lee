...
1. Use the function ```c``` to create a vector with the average high temperature in January for Beijing, Lagos, Paris, Rio d e Janeiro, san Juan, and Toronto, which are 35,88,42,84,81, and 30 degrees Fahrenheit. Call the object ```temp```.
...
ind
states[ind]
my_df <- data.frame(states[ind],sort(pop))
my_df
```
...
temp_cel
city_temps <- data.frame(city, temp, temp_cel)
city_temps
```
...
Now we can look ate the murder rates:
```{r}
murder_rate[ind]
```
...
```{r}
ind <- which (murders$abb == c("Ak", "MI", "IA"))
murders$state[ind]
```
...
```{r}
total_gun_murders_log <- log(base = 10, total_gun_murders)
